...
| 外掛 | 用途 |
|---|---|
| `office-cli` | 以固定版型樣式產出簡報、報告與工作表 —— 五個 skill、五套可互換的設計範本。 |
...
| **SKS Dark** | 午夜藍 | 琥珀色 `#B57319` | 螢幕優先的內容 —— 現場簡報、產品與平台導覽、發表與展示、維運看板 |
| **SKS Blue** | 白色**與**午夜藍 | 藍色 `#2F55F0` | 產品與平台內容 —— 需與線上產品網站並置，同時提供淺色與深色兩個檔案 |

五者採用相同的 19 個具名版面配置，同樣的 1440 × 810 pt 格線，因此在範本之間切換屬於改樣式，而非重新製作。五者皆依 60-30-10 的面積比例建構，且所有色彩對比值一律由程式產生至 `references/contrast-matrix.md`，不以人工填寫。

**SKS Blue** 是唯一擁有兩個底色的一套：白底頁與近黑頁共用同一個強調色、同一套格線、同一組圖表色階。三個分隔版面在兩個檔案中完全相同，因此切換時只有內容頁會變。圖表色階刻意壓在同一明度，讓任何一個序列在切換前後都不改變色相 —— 代價是兩個主題下都沒有任何一個序列色可以當文字。

**SKS Dark** 把整套邏輯翻轉到深色底：各層色階由午夜藍往上走，而非由白色往下走；強調色則以同一暖色的三個明度層次運作 —— 琥珀色作為面積、金色作為文字、深琥珀作為滿版分隔頁底色。深色底上沒有任何單一暖色能同時勝任這三種角色而仍維持可讀性。
